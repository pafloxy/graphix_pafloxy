...
class GraphVisualizer:
    ...
    def visualize_w_gflow(self, g, l_k, angles=None, local_clifford=None, figsize=None, save=False, filename=None):
        ...
        #         k = f[k]
        #         pos[k][1] = i
        layer_data = process_layer_data(self.g, l_k)
        nx.set_node_attributes(self.g, layer_data['attribute_data'])
        pos = nx.multipartite_layout(self.g, subset_key= 'layer', align= 'vertical')
        ...
        # Draw the arrows
        for a, b in g.items():
            nx.draw_networkx_edges(self.g, pos, edgelist=[(a, c) for c in b], edge_color="black", arrowstyle="->", arrows=True)

        # Draw the dashed edges
        for edge in self.g.edges():
            if edge[1] not in g.get(edge[0]) and edge[0] not in g.get(edge[1]) :  # This edge is not an arrow
                intersect = False
                bezier_path = [pos[edge[0]]]
                for node in self.g.nodes():
                    if (
